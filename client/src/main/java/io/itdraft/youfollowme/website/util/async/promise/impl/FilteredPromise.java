...
public class FilteredPromise<D, F, P, D_OUT, F_OUT, P_OUT> extends DeferredObject<D_OUT, F_OUT, P_OUT> {
    ...
    public FilteredPromise(
            final Promise<D, F, P> promise, final DoneFilter<D, D_OUT> doneFilter,
        ...
        promise.done(new DoneCallback<D>() {
            ...
            public void onProgress(P progress) {
                FilteredPromise.this.notify(
                        FilteredPromise.this.progressFilter.filterProgress(progress));
            }
        });
    }
